...
class BandwidthMonitor(app_manager.RyuApp):
	...
	edge_switches = self.topo.edgeSwitches
	hosts = self.topo.hosts

	if name in edge_switches:
	    for stat in body:
                if stat.port_no in self.topo.ports[name].keys():
                    switch_name = self.topo.ports[name][stat.port_no]
		    if switch_name in hosts:
	                total_transmitted = total_transmitted + stat.tx_bytes
                        total_received = total_received + stat.rx_bytes
                        self.bwstats.addHostBwStat(switch_name, total_transmitted, total_received)
